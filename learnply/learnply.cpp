...
int main(int argc, char *argv[])
{
  ...
  progname = argv[0];
	...
	this_file = fopen("../tempmodels/bunny.ply", "r"); // dod & hexahedron have some loading issue
	poly = new Polyhedron (this_file);
	fclose(this_file);
	mat_ident( rotmat );	

	initialize_system();//previous project
	...
	glutCreateWindow ("Geometric Modeling _ AH");
	init ();
...
void init(void) {
  ...
  glClearColor (0.5, 0.0, 0.0, 0.0);  // background
...
double Parity(unsigned int n) // prev
{
	if (n % 2 == 0)
		return 0; // " is even.";
	else
...
void display_shape(GLenum mode, Polyhedron *this_poly)
{
  ...
  glPolygonOffset (1., 1.);
	...
	for (i = 0; i < this_poly->ntris; i++) {
		...
		{
			// p3-1
			int length = 3 * poly->ntris;
			Corner* corners = new Corner[length];
			//std::vector<Corner*> cv(length);//int ntris = poly->ntris;

			for (int i = 0; i < (poly->ntris) - 1; i++) {
				for (int j = 0; j < 2; j++) {
...
void display_irre_ver() {
	for (int i = 0; i < poly->nverts; i++) { //max_verts; i++) {
		...
		int num_edges = 0;
		for (int j = 0; j < poly->nedges; j++) { //max_edges and max_verts can be out of sync

			int valence = 0;
			if (poly->elist[j]->verts[0] == tempv || poly->elist[j]->verts[1] == tempv)
			{
				valence++;
			}
			int defi = 6 - valence;
...
void irreg_reg_valence(int valence) {
	//irreg_vert not eq to 6
	int	reg_vert = 6;
	if (reg_vert >= valence) {
		//blue sphere
	}
	else if (reg_vert <= valence) {
		//red sphere
	}
	else {//reg no dot
...
a = corner.prev.edge.length;
b = corner.next.edge.length;
c = corner.next.next.edge.length;
